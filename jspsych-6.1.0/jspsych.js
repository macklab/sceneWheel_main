...
window.jsPsych = (function() {
  ...
  function checkExclusions(exclusions, success, fail){
    ...
    // WEBP supporting browser
    if(typeof exclusions.webp !== 'undefined' && exclusions.webp) {      
      Modernizr.on('webp', function(result) {
        if (result) {          
          // supported
        } else {
          clear = false;
          var msg = '<p>Your browser does not support the Webp image format,<br> which means that you will not '+
              'be able to complete the experiment.</p>'
          core.getDisplayElement().innerHTML = msg;
          fail();  
          return;                           
        }
